...
public class GpioTest {
    ...
    public static void gpioOutputsMenu(){
        ...
        switch(PlatformManager.getPlatform()){
            case RASPBERRYPI:{
                LedPin pins[] = RaspiPin.allPins();
                Arrays.sort(pins);
                gpioOutputsTest(pins);
                break;
            }
            case BANANAPI:{
                LedPin pins[] = BananaPiPin.allPins();
                Arrays.sort(pins);
                gpioOutputsTest(pins);
                break;
            }
            case BANANAPRO:{
                LedPin pins[] = BananaProPin.allPins();
                Arrays.sort(pins);
                gpioOutputsTest(pins);
                break;
            }
			case BPI:{
                LedPin pins[] = BpiPin.allPins();
                Arrays.sort(pins);
                gpioOutputsTest(pins);
                break;
			}
			case NANOPI:{
                LedPin pins[] = NanoPiPin.allPins();
                Arrays.sort(pins);
                gpioOutputsTest(pins);
                break;
			}
			case ODROID:{
                LedPin pins[] = OdroidC1Pin.allPins();
                Arrays.sort(pins);
                gpioOutputsTest(pins);
                break;
			}
			case ORANGEPI:{
                LedPin pins[] = OrangePiPin.allPins();
                Arrays.sort(pins);
                gpioOutputsTest(pins);
                break;
			}
    ...
    public static void gpioOutputsTest(LedPin pins[]){
        final Scanner in = new Scanner(System.in);
        final GpioController gpio = GpioFactory.getInstance();
        for(LedPin pin : pins){
            // provision pin
